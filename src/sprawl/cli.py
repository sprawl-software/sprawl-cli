...
def get_parser() -> argparse.ArgumentParser:
    ...
    # update
    update_parser = subparsers.add_parser("update", help="Auto-updates both the Global DNA and the local Sprawl CLI engine.")
    update_parser.add_argument("--dry-run", action="store_true", help="Safety mode; simulates update without modifying packages.")
...
def main() -> None:
    ...
    if args.verbose or "--verbose" in unknown or "-v" in unknown:
        config.verbose = True
        from rich.traceback import install as install_traceback
        install_traceback(show_locals=True, theme="monokai")
    if args.dry_run or "--dry-run" in unknown:
        config.dry_run = True
    if args.json or "--json" in unknown:
